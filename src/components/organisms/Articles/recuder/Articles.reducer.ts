...
export const reducer = (state: ArticlesState = initialArticlesState, action: { type: string, payload?: any }) => {
    const {type, payload} = action;
    switch (type) {
        case ArticlesActionType.TOGGLE_CATEGORY: {
            const index = state.categories.findIndex((category) => category === payload);
            if (index > -1) {
